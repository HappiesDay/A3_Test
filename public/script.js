const dragItem = document.getElementById('audio-wrapper');
        ...
        function drag(e) {
            if (active) {
                e.preventDefault(); // Prevent default event (to avoid text selection etc.)
 ...
 if (event.button === 0) {
     leftMouseDown = true;
     rightMouseDown = false; // Ensure right mouse behavior is not triggered
 }
 // detect middle and right clicks
 else if (event.button === 1 || event.button === 2) {
     rightMouseDown = true;  // Treat middle mouse like right mouse
     leftMouseDown = false; // Ensure left mouse behavior is not triggered
 }
...
function onMouseUp(event) {
 // Reset states on mouse up for both middle and right mouse buttons
